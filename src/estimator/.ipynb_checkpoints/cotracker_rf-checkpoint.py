...
def estimate_rf_from_cotracker(
    frame_dir: str,
    camera: str,
    fps: float,
    roi_region: str = "chest",
    with_pca: bool = False,
    lo_hz: float = 0.07,
    hi_hz: float = 1.0,
    grid_size: int = 30,
    ...
    """
    Estimate respiratory rate (Rf) from video frames using CoTracker motion tracking.
    This function loads a directory of PNG frames, extracts a chest/abdomen ROI
    (or an optional person-segmented ROI), runs CoTracker to obtain dense point
    trajectories, and converts the tracked motion into a 1D breathing waveform
    (median vertical displacement or PCA-based motion component).
    The resulting waveform is detrended, band-pass filtered in the breathing band,
    smoothed, and analyzed via Welch PSD to estimate RR in breaths-per-minute.
    It also extracts a breath-amplitude proxy by pairing peaks/troughs in the
    filtered waveform.

    Args:
        frame_dir (str): Directory containing input frames as PNG images.
        camera (str): Camera identifier.
        fps (float): Frame rate of the input video in fps (Hz).
        roi_region (str): ROI selection mode/region ("chest", "abdomen" or "segmented").
        with_pca (bool): If True, extract the breathing waveform using PCA on tracked
                        displacements instead of median vertical displacement.
    ...
    Returns :
        rr_video_bpm (float) : Estimated respiratory rate in bpm (NaN on failure).
        sig_smooth (ndarray) : Final filtered + smoothed breathing waveform.
        sig_raw (ndarray) : Raw displacement-based signal before filtering.
        t_amp (ndarray) : Breath amplitude timestamps (seconds, relative to video start).
        amp_proxy (ndarray) : Amplitude proxy per detected breath event.
    """
