...
def load_cosmed_take(xlsx_path, take, sheet="Data", resample_dt_s=None):
    ...
    Args:
        xlsx_path (str): Path to the COSMED Excel file.
        take (str): Marker identifier (e.g. "take1") to extract.
        sheet (str): Excel sheet name containing the data.
        resample_dt_s (float|None): If provided, resample signals to this step (seconds).

    Returns:
        out (pd.DataFrame): A DataFrame with columns ["time_s", "Rf", "VT", "VE"].
    """
